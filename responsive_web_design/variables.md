...
The CSS Variable format:

__--penguin-skin: gray;__

The reference format:

__background: var(--penguin-skin, black);__

Where _black_ is a a fallback value that your browser will revert to if the given variable is invalid.

Note: this fallback is not used to increase browser compatibilty, and it will not work on IE browsers. 
Rather, it is used so that the browser has a color to display if it cannot find your variable.

In order to provide a browser fallback, provide another more widely supported value immediately BEFORE your declaration. That way an older browser will have something to fall back on, while a newer browser will just interpret whatever declaration comes later in the cascade.
